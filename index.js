...
const settings = {
  cubeRotation: '180', // 0-359
  interpolation: 'lanczos', // cubic, linear
  format: 'png',  // jpb lanczo,w RadioInput('format', loadImage),
};
...
async function renderFace({data: readData, face, rotation, interpolation, maxWidth = Infinity}) {
  ...
  // Code expects alpha channel...
  const faceImage = await sharp({
    create: {
      width: faceWidth,
      height: faceHeight,
      channels: 4,
  ...
  const faceData = new Uint8ClampedArray(faceImage.buffer);
  const writeData = {
    width: faceWidth,
    height: faceHeight,
    data: faceData,
  ...
  return { name: face, ...writeData};
}

async function processFace(data, pathPrefix, faceName) {
  const options = {
    data: data,
    face: faceName,
  ...
  const { data: resultBuff, width, height } = await renderFace(options);
  return sharp(resultBuff,{
    raw: {
      height,
      width,
      channels:4,
    }
  }).toFile(`${pathPrefix}_${faceName}.png`);
}

async function processImage(imageFileName, outDir='.', prefix=undefined) {
  const sharpImg = sharp(imageFileName)
  // write war pixel data to a buffer
  const { width, height, channels } = await sharpImg.metadata();
  ...
  const data = await sharpImg
      ...
      .raw()
      .toBuffer();
  const pathPrefix= path.join(outDir, prefix ?? path.parse(imageFileName).name);
  const faces = Object.keys(orientations)
